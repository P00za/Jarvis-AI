...
#Asynchronus function to conver text to an audion file
async def TextToAudioFile(text)-> None:
    file_path = r"Data\speech.mp3"  # define the path where the speech file will be saved.

    if os.path.exists(file_path): # Check if the file already exists.
        os.remove(file_path)  # If it exist , remove it to avoid overwritting erroes.

    # Create the cummunicate object to generate speech
    communicate = edge_tts.Communicate(text,AssistantVoice, pitch='+5Hz', rate='+13%') 
    await communicate.save(r'Data\speech.mp3')  # save the generted speech as an MP3 filw.
...
def TTS(Text,func=lambda r=None: True):
    while True:
        try:
            ...
            pygame.mixer.music.play() # Playb the audio.
            ...
            try:
                # Call the provided functioin wiith false to signal the end of tts
                func(False)
                pygame.mixer.music.stop() # To stop the audio playback
